...
class EmpleadosStreamTests {
	...
	void test1() {
		...
		try {
			...
			//
			List<Integer> listaCodigos = listEmp.stream()
													.map(e -> e.getCodigo())
													.distinct()
													.collect(toList());
	...
	void test2() {
		...
		try {
			...
			//
			List<String> listaNombreApellido = listEmp.stream()
														.filter(t -> t.getApellido2()!=null)
														.map(e -> "Nombre: "+ e.getNombre().toUpperCase() + ", Apellido 1: " + e.getApellido1().toUpperCase() + ", Apellido2 : " + e.getApellido2().toUpperCase())
														.collect(toList());
	...
	void test3() {
		...
		try {
			...
			List<String> listaCodigoNif = listEmp.stream()
												.map(e -> "Código Empleado: " + e.getCodigo() + ", Nif: " + e.getNif().substring(0, 8) + ", Letra: " + e.getNif().charAt(8))
												.collect(toList());
			//
			
			listaCodigoNif.forEach(System.out::println);
	...
	void test4() {
		...
		try {
			...
			//	
			List<String> listaDeparValor = listDep.stream()
													.map(d -> "Nombre: " + d.getNombre() + ", Presupuesto actual: " + (d.getPresupuesto()-d.getGastos()))
													.collect(toList());
			
			listaDeparValor.forEach(System.out::println);
	...
	void test6() {
		...
		try {
			...
			//
			List<String> listaDeparValor = listDep.stream()
					.sorted(comparingDouble(Departamento::getPresupuesto).reversed())
	...
	void test11() {
		...
		try {
			...
			List<String> listaNombre = listEmp.stream()
											.filter(t -> t.getApellido2()!=null)
											.filter(t -> t.getDepartamento()!=null)
											.filter(t -> t.getDepartamento().getCodigo().equals(2) || t.getDepartamento().getCodigo().equals(4) || t.getDepartamento().getCodigo().equals(5))
											.map(e -> "Nombre: " + e.getNombre() + ", Apellido 1: " + e.getApellido1() + ", Apellido 2: " + e.getApellido2() + ", DNI: " + e.getNif())
											.collect(toList());
	...
	void test12() {
		...
		try {
			...
			List<String> lista = listEmp.stream()
										...
										.map(e -> "El empleado con dni 38382980M trabaja en: "+ e.getDepartamento().getNombre())
										.collect(toList());
	...
	void test13() {
		...
		try {
			...
			List<String> lista = listEmp.stream()
									...
									.map(e -> "Nombre: " + e.getNombre() + ", Apellido 1: " + e.getApellido1() + ", Apellido 2: " + e.getApellido2())
									.collect(toList());
	...
	void test14() {
		...
		try {
			...
			//
			List<String> lista = listDep.stream()
												.sorted(comparingDouble(Departamento::getPresupuesto))
												.limit(1)
												.map(d -> "El presupuesto mínimo es de: " + d.getPresupuesto())
												.collect(toList());
